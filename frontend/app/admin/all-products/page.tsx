...
function ProductsContent() {
      ...
      if (response.success) {
        setProducts(products.map(product => 
          product._id === selectedProduct._id ? { 
            ...product, 
            ...editFormData,
            images: response.data?.images || product.images
          } : product
        ));
  ...
  const hasActiveFilters = searchQuery || selectedCategory || stockFilter;
    ...
    <div className="min-h-screen bg-linear-to-br from-blue-900 via-purple-900 to-black">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-4 mb-6">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Search products by name, company, flavour..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="block w-full pl-10 pr-3 py-2.5 bg-white/10 border border-white/20 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
              />
            </div>
            <button
            ...
            >
              ...
              {hasActiveFilters && (
                <span className="ml-1 bg-red-500 text-white text-xs px-1.5 py-0.5 rounded-full">
                  {[searchQuery ? 1 : 0, selectedCategory ? 1 : 0, stockFilter ? 1 : 0].reduce((a, b) => a + b, 0)}
                </span>
              )}
            </button>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="block w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-red-500"
                ...
                <div className="flex items-end">
                  <button
                    ...
                    className="w-full px-4 py-2 text-sm font-medium text-gray-300 bg-white/10 border border-white/20 rounded-lg hover:bg-white/20 transition-colors"
                  >
                    Clear Filters
                  </button>
                </div>
        ...
        {isLoading ? (
          ...
          <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-8 text-center">
            <div className="text-gray-400 text-lg">
              <p>{hasActiveFilters ? "No products match your filters" : "No products available"}</p>
            </div>
          </div>
        ) : (
